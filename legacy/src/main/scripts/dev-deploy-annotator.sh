...
SRCFILE=$HOME/annotator/target/annotator.war
...
if [ ! -f $SRCFILE ]; then
    echo "$SRCFILE does not exist - did you forget to build?"
    exit 3
fi
...
echo "Service started - copying war file to iistdc as $(whoami)"
scp $SRCFILE iistdc.memphis.edu:~
