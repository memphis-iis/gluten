...
class OAuthRemoteApp(object):
    ...
    :param access_token_method: the HTTP method that should be used
                                ...
                                to ``'GET'``.
    :param bearer_auth_header: an parameter to automatically add
                an OAuth2 authentication header.
                Checkout: http://tools.ietf.org/html/rfc6750#section-2.1
                Defaults to False
    :param bearer_auth_header_prefix: the word between Authorization:
    ...
    def __init__(self, oauth, name, base_url,
        ...
        self.bearer_auth_header_prefix = bearer_auth_header_prefix
    ...
    def request(self, url, data="", headers=None, format='urlencoded',
        ...
        if self.bearer_auth_header:
            headers['Authorization'] = '%s %s' % (
                self.bearer_auth_header_prefix,
                client.token.key
            )
    ...
    def get_request_token(self, token=None):
        assert self.tokengetter_func is not None, 'missing tokengetter func'
        ...
        if rv is None:
            ...
            if rv is None:
                raise OAuthException('No token', type='token_missing')
    ...
    def authorize(self, callback=None, extra_params={}):
        ...
        case the application will most likely switch to PIN based authn
